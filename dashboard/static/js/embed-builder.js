...
window.onload = () => {
    ...
    let editorHolder = document.querySelector('.editorHolder'),
        ...
        toRGB = (hex, reversed, integer) => {
            ...
            content: "This is a Message Embed <a:R2:906313693623640085>\nEverything is _Customizable_ including **Emojis**\nYou can~~not~~ also do `this`.```py\nAnd ('this')```",
            embed: {
                title: "Hello! :wave:",
                description: "You can use [links](https://rover.deadpool296.repl.co/) or emojis :smile: :eyes:\n```\nAnd also code blocks\n```",
                color: 4321431,
                timestamp: new Date().toISOString(),
                url: "https://rover.deadpool296.repl.co/",
                author: {
                    name: "Author name",
                    url: "https://rover.deadpool296.repl.co/",
                    icon_url: "https://i.imgur.com/d7iStJL.png"
                },
                thumbnail: {
...
if(command.length > 30) return error("Command length exceeds 30 characters.", 4000);
  ...
  error('Successfully saved command, if not saved, <a href="https://rover.deadpool296.repl.co//embeds">read here</a>', 10000)
        e.preventDefault();
        $.ajax({
            type: "POST",
