...
app.use(session({
  ...
  store :MongoStore.create({ mongoUrl: jsonconfig.mongodb_url})
    ...
    app.get("/callback", passport.authenticate("discord", {
      ...
      const loginLogs = new Discord.WebhookClient(jsonconfig.webhook_id, jsonconfig.webhook_url);;
...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/SbYUMuVV5F`)
    });

    app.get("/server", (req, res) => {
      res.redirect(`https://discord.gg/SbYUMuVV5F`)
    });
    app.get("/Rover music", (req, res) => {
      res.redirect(`https://discord.com/api/oauth2/authorize?client_id=905623123796307998&permissions=8&scope=bot`)
    });
    ...
    app.get("/logout", async function(req, res) {
      ...
      const member = await client.users.fetch(req.user.id);
        if (member) {
          ...
          const logoutLogs = new WebhookClient(jsonconfig.webhook_id, jsonconfig.webhook_url)
...
member.send(new MessageEmbed().setColor('GREEN').setFooter(`Powered by https://rover.deadpool296.repl.co/`).setTitle(`Application #${ticketID}`).setDescription(`Hey ${member.user.username}! Your form was Submitted and ready to be judged.\n\n**Form ID**: \`${ticketID}\`\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)).catch(()=>{});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('878722123303108639');
...
if(results.length >= 10) return;
    ...
    app.post("/report", async (req, res) => {
      ...
      if (req.body.type === "report") {
        const reportEmbed = new WebhookClient(jsonconfig.webhook_id, jsonconfig.webhook_url);

        const report = new MessageEmbed()
          .setColor('GREEN')
          .setTitle(`Rover Reports`)
    ...
    app.post("/contact", async (req, res) => {
      ...
      if (req.body.type === 'contact') {
        ...
        const contactEmbed = new WebhookClient(`907879659339923506`,`c9KiwgLUolA-CxTwuOdNftDSMV5OcK-0hKoIa-zfJ5w1IKBOkbPTuiSQkVi4tWom7MGs`);
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            if (storedSettings.isPremium == "false") {
              ticketSettings.ticketFooter = "Powered by https://Rover.deadpool296.repl.co";
            } else {
            ...
            let reactionFooter = "Powered by https://Rover.deadpool296.repl.co";



            let footer = "Powered by https://Rover.deadpool296.repl.co";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by https://Rover.deadpool296.repl.co`)
            } else {
...
if(maintenance && maintenance.toggle == "true") {
    ...
    app.post('/dblwebhook', webhook.middleware(), async (req) => {
      ...
      const msg = new Discord.MessageEmbed()
        ...
        .setDescription(`Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **Rover**!`)
      Hook.send(msg);
...
if(config.datadogApiKey){
  metrics.init({ apiKey: jsonconfig.datadogApiKey, host: 'Rover', prefix: 'Rover.' });
