...
if(!suggestionName) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggest1}`).setFooter('https://Rover.deadpool296.repl.co').setTimestamp().setColor('RED'));
...
if(guildDB.suggestion.suggestionChannelID !== null){
  ...
  if(channelLog){
   ...
   icon: `https://i.imgur.com/d7iStJL.png`
 }).catch(()=>{})
  }
}
