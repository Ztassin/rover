...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    const errorEmbed = new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png'})).setColor(client.color.red).setFooter("https://Rover.deadpool296.repl.co").setTimestamp()
    const successEmbed = new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png'})).setColor(client.color.green).setFooter("https://Rover.deadpool296.repl.co").setTimestamp()
