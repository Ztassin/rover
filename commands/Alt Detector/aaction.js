...
module.exports = class extends Command {
  ...
  if(!args[0]) return message.channel.send(new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} ${language.aactionNotValidChoice.replace("{allChoices}", choices.join(", "))}`).setFooter('https://Rover.deadpool296.repl.co').setTimestamp().setColor('RED'));
  ...
  await alt.findOne({ guildID: message.guild.id }, async (err, db) => {
            ...
            return message.channel.send(new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.success} ${language.aactionSuccess.replace("{action}", args[0])}`).setFooter('https://Rover.deadpool296.repl.co').setTimestamp().setColor('RED'));
          }
      await db.updateOne({
        altAction: args[0].toLowerCase()
      })
