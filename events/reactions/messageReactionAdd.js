...
.setAuthor('Role Added', `https://i.imgur.com/d7iStJL.png` , `${message.url}` )
...
.setAuthor('Role Removed', `https://i.imgur.com/d7iStJL.png` , `${message.url}` )
.setDescription(`You have removed the **${rrRole.name}** Role by reacting in ${guildName}`)
.setFooter(`https://Rover.deadpool296.repl.co`)
.setColor(message.client.color.green)

let errorReaction = new MessageEmbed()
.setAuthor('Reaction Role Error', `https://i.imgur.com/d7iStJL.png` , `${message.url}` )
...
let arraylength = array.length
      ...
      if(arraylength > ticketlimit || arraylength == ticketlimit) {
        ...
        message.channel.send(new discord.MessageEmbed().setColor(message.client.color.red).setDescription(`You already have ${arraylength} open tickets, as the current guild's ticket limit is ${ticketlimit} `).setAuthor(user.tag, user.displayAvatarURL()).setFooter('https://Rover.deadpool296.repl.co')).then(m => m.delete({timeout: 5000}));
...
let color2 = db.ticketLogColor
    if(color2 == "RANDOM") color2 = `RANDOM`;
