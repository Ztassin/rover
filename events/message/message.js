    ...
    metrics.init({ apiKey: this.client.config.datadogApiKey, host: 'Rover', prefix: 'Rover.' });
      ...
      if (message.content.match(mentionRegex)) {
        ...
        const embed = new MessageEmbed()
          .setTitle('Hello, I\'m Rover. What\'s Up?')
          .addField(`Prefix`,proofita, true)
          .addField(`Usage`,proofitaa, true)
          .setDescription(`\nIf you like Rover, Consider, or [inviting](https://discord.com/api/oauth2/authorize?client_id=849022851271884880&permissions=8&scope=bot) it to your server! Thank you for using Rover, we hope you enjoy it, as we always look forward to improve the bot`)
          .setFooter('Thank you for using Rover!!')
          .setColor('RANDOM')
...
message.channel.send(`Rover is currently undergoing maintenance which won't allow anyone to access Rover's Commands. Feel free to try again late
r. For updates: https://discord.gg/SbYUMuVV5F`)
...
return message.channel.send(embed)
  ...
  if (userBlacklistSettings && userBlacklistSettings.isBlacklisted)  return;
      ...
      if (command) {
        ...
        if (typeof rateLimit === "string") return message.channel.send(` ${message.client.emoji.fail} Please wait **${rateLimit}** before running the **${cmd}** command again - ${message.author}\n\n${number === 1 ? "*Did You know that Rover has its own dashboard? `https://Rover.deadpool296.repl.co/dashboard`*" : ""}${number === 2 ? "*You can check our top.gg page at `https://vote.Rover.xyz`*" : ""}`).then((s)=>{
