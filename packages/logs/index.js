module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'https://Rover.deadpool296.repl.co')
    if (!hook) {
        try {
            hook = await channel.createWebhook('https://Rover.deadpool296.repl.co', {
                avatar: `https://Rover.deadpool296.repl.co/logo.png`
            });
        } catch (e) {
            hook = await channel.createWebhook('https://Rover.deadpool296.repl.co', `https://Rover.deadpool296.repl.co/logo.png`);
        }
